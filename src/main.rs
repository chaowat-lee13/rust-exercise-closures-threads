...
fn main() {
    ...
    // Code for challange
    // Create the first channel for child thread A
    let (tx_a, rx_a) = channel::unbounded();
    // Create the second channel for child thread B
    let (tx_b, rx_b) = channel::unbounded();

    // Spawn child thread A
    let handle_a = thread::spawn(move || {
        for received in rx_a {
            println!("Child thread A received: {}", received);
        }
    });

    // Spawn child thread B
    let handle_b = thread::spawn(move || {
        for received in rx_b {
            println!("Child thread B received: {}", received);
        }
    });

    // Main thread sends values to both child threads
    let values = ["value1", "value2", "value3", "value4"];
    for value in &values {
        println!("Main thread sending: {}", value);
        tx_a.send(value.to_string()).unwrap();
        tx_b.send(value.to_string()).unwrap();
